...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('update_popularity.php', {
        method: 'POST',  // Use POST instead of GET for data modification
        headers: {
            'Content-Type': 'application/json',  // Optionally send JSON data if needed
        },
        body: JSON.stringify({ action: 'update_popularity' })  // Sending a JSON payload if needed
    })
    .then(response => response.text())
    .then(text => {
        console.log("Popularity Update Response: ", text);
    })
    .catch(error => {
                ...
                <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                    ${products.map(product => `
                        <div class="bg-white border border-gray-300 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
                            <img src="uploaded_img/${product.image}" alt="${product.name}" class="w-full h-48 object-cover rounded-t-lg" />
                            <div class="p-4">
